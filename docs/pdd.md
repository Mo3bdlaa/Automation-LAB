...
**Version:** 1.0 · **Date:** 2026-09-05 · **Author:** Mohammed Shaker · **Status:** Draft for review
...
| Item | P1 Invoice processing with three-way match | P2 Vendor onboarding from commercial licence | P3 Purchase orders awaiting invoice |
| --- | --- | --- | --- |
...
### 2.3 Detailed AS-IS steps: P1 Invoice processing
...
### 2.4 Detailed AS-IS steps: P2 Vendor onboarding
...
### 2.5 Detailed AS-IS steps: P3 Purchase orders awaiting invoice
...
4. Run Document Understanding: digitize, classify as Tax Invoice, extract header and line fields with the taxonomy in Appendix C. Route to Validation Station when confidence is below 0.85 on any critical field (number, grand total, IBAN, PO number).
...
P0 (foundation) and P1 (full document cycle, seeded defects, three-way match) are implemented. The remaining phases are defined here with deliverables, the exercise each unlocks, and acceptance criteria that the smoke test and unit tests must cover before the phase is called done. P4 is new: it gathers the production and integration items that the design left open.

### 4.1 P2: Queues, REST API, grading, validation station, instructor dashboard
