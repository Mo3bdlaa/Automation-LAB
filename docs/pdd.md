...
Three processes are automated during the course, each in a UI variant and later an API variant. All three run at Automation Lab Trading & Contracting Co., the fictional company every student works for. Volumes below are per student sandbox at provisioning; the figures grow as students create records.
...
| 1.5 | 2026-09-09 | Mohammed Shaker | Ladder re-measured over 30 invoices and the Arabic result corrected: the cost is the Eastern Arabic-Indic numerals, not the Arabic script. Acceptance check on the ladder made a paired test against sampling error. |
