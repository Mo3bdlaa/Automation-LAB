/**
 * Builds the lab-wide Process Definition Document (PDD) in three formats from
 * one content model:
 ...
 * The emitters live in scripts/pdd/render.ts; this file is the words.
 *   pnpm pdd
 */
...
const DOC: DocMeta = {
  ...
  strapline: "Document Understanding & RPA course · practice sandbox",
...
const B: Block[] = [
  ...
  { t: "p", text: "Three processes are automated during the course, each in a UI variant and later an API variant. All three run at Automation Lab Trading & Contracting Co., the fictional company every student works for. Volumes below are per student sandbox at provisioning; the figures grow as students create records." },
  ...
  { t: "pagebreak" },
  { t: "h1", text: "4. Platform roadmap: phases P2, P3, P5 and P4" },
  ...
  { t: "bullets", items: [
    ...
    "Met, with a finding. pnpm ocr:ladder renders every level, reads page 1 back with Tesseract and reports the share of ground-truth values the OCR output contains. On a 30-invoice sample at 300 dpi: L1 79.0 per cent, L2 78.8, L3 76.5, L4 62.6, L5 51.5, within a point or two of an earlier 15-invoice run. The check that no level reads better than the one before it is paired per document and measured against the sampling error rather than a fixed margin, because the same documents are read at every level and a population this spread out moves several points between runs on noise alone. The finding is that a clean synthetic scan costs OCR almost nothing: over 24 invoices the steps are L1 to L2 minus 1.0 points against a two standard error band of 1.0, L2 to L3 minus 2.4 (2.0), L3 to L4 minus 10.9 (4.1) and L4 to L5 minus 7.5 (5.7). The ladder's weight is in levels 4 and 5, where perspective and shadow start to bite; the real step for a bot is the one this test cannot see, level 1 to level 2, where the text layer disappears and extraction has to become OCR at all.",
    ...
    "Met on Arabic too, and the numbers say something the course should teach. It is not Arabic that costs recall - it is the numeral system. Measuring every one of the 105 Arabic-first invoices at level 1: those from vendors printing Western digits score 91.4 per cent overall and 87.5 per cent on the numeric fields, while those printing Eastern Arabic-Indic digits score 27.2 per cent overall and 2.9 per cent on the numeric fields. An Arabic-first document set in Western digits therefore reads as well as an English one (91.4 against 93), and Tesseract with ara+eng essentially cannot read Eastern digits at all: 2.9 per cent is a floor rather than a degradation, and the 27 per cent that survives is the Arabic words still being read around numbers that are lost. An earlier reading of a mixed sample, that Arabic costs about half the recall, was wrong - it averaged a bimodal population into one number, and the ladder tool now reports the two Arabic groups separately so the same mistake is not available. About two Arabic-first vendors in five print Eastern digits and about three vendors in ten print Arabic-first, so this reaches roughly one document in eight: a difficulty spike rather than a wall, and the bilingual documents that make up half the corpus always print both scripts with the alternate readings recorded in ground truth. Which OCR or extraction engine to point at a document, and how to configure it, is the student's decision and part of the exercise; a student who notices the digits and switches engine, adds a digit-aware pass or folds the numerals before matching recovers almost everything, and one who concludes that Arabic OCR is hopeless gets nowhere. The lab's job is to produce documents that make the decision matter, and it does.",
  ] },
  ...
  { t: "h2", text: "4.3 P5: The public challenge - delivered" },
  { t: "p", text: "The lab was built for a cohort. P5 wraps the same sandbox in a scored, timed and verifiable challenge that a stranger can find, complete and prove they completed, so that it can be run as a community hackathon rather than only as coursework." },
  { t: "h3", text: "Deliverables" },
  { t: "table", header: ["Area", "Deliverable", "Detail"], widths: [1900, 3200, 4260], rows: [
  ...
  { t: "h2", text: "4.4 P4: Production identity, deployment and integration" },
  { t: "h3", text: "Deliverables" },
  { t: "table", header: ["Area", "Deliverable", "Detail"], widths: [1900, 3200, 4260], rows: [
  ...
  { t: "h2", text: "4.5 Dependencies and order" },
  ...
  { t: "table", header: ["Version", "Date", "Author", "Change"], widths: [1200, 1600, 2600, 3960], rows: [
    ...
    ["1.4", "2026-09-08", DOC.author, "P5 delivered: self-service accounts, four scored scenarios, five judging parameters, opt-in leaderboard, verifiable certificates, per-scenario PDD and SDD, and the run API. New section 4.3; the P4 section renumbered to 4.4."],
    ["1.5", "2026-09-09", DOC.author, "Ladder re-measured over 30 invoices and the Arabic result corrected: the cost is the Eastern Arabic-Indic numerals, not the Arabic script. Acceptance check on the ladder made a paired test against sampling error."],
  ] },
...
buildPdd({
  doc: DOC,
  blocks: B,
  outBase: ".data/Automation-Lab-PDD",
  markdown: {
    file: "docs/pdd.md",
    notes: [
  ...
  .then(({ docx, pdf, pages }) =>
    console.log(`Wrote docs/pdd.md, .data/Automation-Lab-PDD.docx (${docx} bytes) and .data/Automation-Lab-PDD.pdf (${pages} pages, ${pdf} bytes); ${rules.length} rules in the catalogue`),
  )
  .catch((e) => {
    console.error(e);
    process.exit(1);
