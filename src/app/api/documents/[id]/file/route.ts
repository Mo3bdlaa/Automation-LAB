...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!file) {
    // Queue the render for kinds that are not produced at provisioning time.
    if (!EAGER_KINDS.has(doc.kind) && level === 1) {
      await enqueue("render_document", { documentId: doc.id }, { tenantId: doc.tenantId, priority: 8 });
      kickJobs();
    }
    return Response.json({ error: "not_rendered", documentId: doc.id, level, message: "The PDF is being rendered. Retry after a few seconds." }, { status: 409, headers: { "Retry-After": "5" } });
  }
